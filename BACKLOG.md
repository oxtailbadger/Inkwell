...
- [x] **Rate limiting + fetch-og caching** — done (2026-07-07), with one deliberate deviation: **in-memory per-instance** (`lib/rate-limit.ts`, `lib/server-cache.ts`) instead of Upstash/Vercel KV, which would require provisioning an external service. Per-instance covers the real beta-scale threat (rapid repeats from one user hit the same warm instance); counters reset on cold starts and aren't shared across instances — swap the Map for Redis behind the same interfaces when the app opens up. Both `/api/fetch-og` and `/api/archive-check` limit 10/min/user (429 + Retry-After; archive-check degrades to `found:false` since it's best-effort everywhere it's consumed). fetch-og caches successes AND stable EPROXYNEEDED antibot blocks by URL for 24h (transient failures retry); archive-check caches found=true only (snapshots are permanent, absence isn't). Cache hits don't count against the limit.
- [x] **CI + broader test coverage** — done (2026-07-07). `.github/workflows/ci.yml`: typecheck + vitest + build (with placeholder Supabase env vars — validated locally that the build passes with them) on push to main and on PRs. Coverage extended 52 → 92 tests: nods toggle (insert/delete/sanitized errors), fetch-og (Microlink 200-on-fail gotcha, EPROXYNEEDED→manual, image-vs-logo mapping, cache + rate-limit behavior), author-articles (paywall filtering via new `lib/paywall.ts`, top-3 cap, per-feed failure isolation, site_icon_url passthrough), paywall heuristics unit tests (incl. the 80-char boundary), profiles enrichment (`fetchEnrichedArticles` nod counts/user state/display names), and the rate limiter itself. Playwright smoke suite against a seeded deploy: not done, still worth considering before public launch.
