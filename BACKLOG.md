...
- [ ] **Profile section** — a header entry point (noted 2026-07-09) for account preferences and help, distinct from the existing inline `ThemeToggle`/Sign out buttons. Should cover: dark mode as a real preference living somewhere a user would look for it (today it's only the header toggle, persisted via `localStorage`, not surfaced as a "setting"); an FAQ section (e.g. "how do I share an article" — walking through the submit form, the `/share` Web Share Target flow, and the archive.is field); and an iOS Shortcuts integration so sharing into Inkwell is a one-tap Shortcuts action, not just the existing PWA `share_target` manifest entry — worth checking whether that alone is already enough for a Shortcuts "Share Sheet" action or whether it needs a dedicated URL scheme/App Intent.
